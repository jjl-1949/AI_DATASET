...
class ResNet50Backbone(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        C2 = self.layer1(x)          # /4,  256c
        C3 = self.layer2(C2)         # /8,  512c
        C4 = self.layer3(C3)         # /16, 1024c
        C5 = self.layer4(C4)         # /32, 2048c
...
class RGBBranch(nn.Module):
    ...
    def __init__(self, frozen_stages: int = 1):
        ...
        self.backbone = ResNet50Backbone(
            in_channels=3, pretrained=True, frozen_stages=frozen_stages)
...
def _test():
    print("=" * 50)
    print("[RGBBranch] ResNet50 transfer learning")
    B, H, W = 2, 640, 640
